...
function PageNavigationTitle({
  children,
  className,
}: PageNavigationTitleProps) {
  return (
  ...
  return (
    <a
      className={cn(
        pageNavigationDefaultAnchorStyle,
        className,
        current ? pageNavigationCurrentAnchorStyle : '',
      )}
      href={link}
    >
      {iconElement && (
        <span className={pageNavigationAnchorIconStyle}>{iconElement}</span>
      )}
...
function PageNavigationAccordionItem({
  children,
  className,
  value,
}: PageNavigationAccordionItemProps) {
  return (
...
function PageNavigationAccordionTrigger({
  text,
  className,
  iconElement,
}: PageNavigationAccordionTriggerProps) {
  return (
    <AccordionTrigger
      className={cn(pageNavigationAccordionTriggerDefaultStyle, className)}
    >
      {iconElement && (
...
function PageNavigationAccordionContent({
  children,
  className,
}: {
  children: React.ReactNode;
  ...
  return (
    <AccordionContent
      className={cn('ml-4 flex flex-col gap-y-2 border-l p-0 pl-4', className)}
    >
      {children}
    </AccordionContent>
  );
